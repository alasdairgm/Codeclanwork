...
```{r}
pattern <- "[A-Z][a-z]+ [0-9]+(th|rd|st|nd), [1-9]{4}"
...
```{r}
pattern <- c("Winston|Smith")

whole_text <-  whole_text %>% 
  str_replace_all(pattern, "REDACTED")

# check for any winstons
whole_text %>% 
  str_detect(pattern)
```
...
whole_text %>% 
  str_extract_all(pattern)
```
Step 2, replace these with vodka
```{r}
whole_text  <- whole_text %>% 
...
My mistake was trying do it all with one pattern, but you can actually just check multiple patterns with pipes
